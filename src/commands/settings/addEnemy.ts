...
export async function execute({ interaction }: CmdFileArgs): Promise<void> {
  ...
  while (groupId === 0) {
    await interaction.editReply({
      content: 'What is the ID of the group you want to add **as an enemy**?'
    });
    // Collect the message
  ...
  if (groupJson.groups.enemies.includes(groupId)) {
    interaction.editReply({ content: 'That group already exists in the enemies list!' });
    return;
  }
  // Add group to enemies
  groupJson.groups.enemies.push(groupId);
  fs.writeFileSync('./configs/groups.json', JSON.stringify(groupJson, null, 2));
  // Send confirmation
  interaction.editReply({
    content: `Added ${groupData.name} to the enemies list!`
  });
  return;
}
